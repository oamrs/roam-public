...
//!
//! This module provides a gRPC server wrapper around
//! the existing QueryServiceImpl and SchemaServiceImpl.
...
/// gRPC executor that wraps QueryService and SchemaService implementations
///
/// Provides a Tonic gRPC server interface to the existing OAM executor logic,
/// enabling polyglot clients (Python, Go, .NET, etc.) to interact with the core.
///
/// # Example
/// ```ignore
/// let executor = GrpcExecutor::new("test.db")?;
/// let _handle = executor.start_server("127.0.0.1:50051").await?;
/// ```
...
impl GrpcExecutor {
    /// Create a new gRPC executor with the given database path
    ///
    /// # Arguments
    ...
    /// Start the gRPC server at the given address
    ///
    /// # Arguments
    /// * `addr` - Server address (e.g., "127.0.0.1:50051")
    ///
    /// # Returns
    /// A JoinHandle to the server task. The server runs until the handle is dropped.
    ///
    /// # Example
    /// ```ignore
    /// let executor = GrpcExecutor::new("test.db")?;
    /// let handle = executor.start_server("127.0.0.1:50051").await?;
    /// // Server is now running...
    /// drop(handle); // Stop the server
    /// ```
...
/// gRPC implementation of QueryService
...
/// gRPC implementation of SchemaService
